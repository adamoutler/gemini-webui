...
_Note: We have also configured `addopts = --timeout=60` in `pytest.ini` as a global fallback, but explicit `@pytest.mark.timeout` annotations are preferred for granular control._
