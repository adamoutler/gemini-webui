...
import { escapeHtml } from "../core/api.js"; // NOSONAR
...
export async function confirmShareSession() {
  const tab = tabs.find((t) => t.id === activeTabId);
  if (!tab || !tab.term) return; // NOSONAR
  ...
  htmlDump = htmlDump.replace(/color:\s*([0-9a-fA-F]{6})/g, "color: #$1"); // NOSONAR
  ...
  while (lines.length > 0) {
    const lastLine = lines[lines.length - 1]; // NOSONAR
    if (
      !lastLine.textContent || // NOSONAR
      !lastLine.textContent.replace(/\u00a0/g, " ").trim() // NOSONAR
    ) {
      lastLine.remove();
      lines.pop();
    } else {
...
function openAddPromptModal(index = -1) {
  currentEditPromptIndex = index; // NOSONAR
...
export async function deletePrompt(index) {
  if (confirm("Are you sure you want to delete this prompt?")) {
    ...
    if (!prompt || !prompt.id) return; // NOSONAR
    try {
