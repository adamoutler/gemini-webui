...
def test_new_connection_clean_startup(page, server):
    ...
    assert "ImportProcessor" not in terminal_text, "Found ImportProcessor error in terminal output"
    assert "reality-checker)" not in terminal_text, "Found reality-checker) parsing error in terminal output"
    assert "Keychain initialization" not in terminal_text, "Found Keychain initialization error in terminal output"
    assert "Invalid session identifier" not in terminal_text, "Found Invalid session identifier error in terminal output"
