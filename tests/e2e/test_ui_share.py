...
def test_ui_share_workflow(page, server, playwright):
    ...
    time.sleep(1)
    expect(page.locator("#share-modal")).to_contain_text(
        "This will generate a publicly accessible link.", timeout=15000
    )
    ...
    if inner_bg is not None:
        assert inner_bg in [
            ...
            "rgb(255, 255, 255)",
            "transparent",
        ], f"Expected inner div to not have hardcoded black background, got {inner_bg}"
