...
def test_share_color_averages(page, server, playwright):
    ...
    def get_share_screenshot(theme):
        ...
        body_bg = new_page.evaluate(
            "window.getComputedStyle(document.body).backgroundColor"
        )
        print(f"\\nComputed body background for theme {theme}: {body_bg}")

        has_inline_bg = new_page.evaluate(
            "!!document.querySelector('.terminal-wrapper [style*=\"background-color\"]')"
        )
