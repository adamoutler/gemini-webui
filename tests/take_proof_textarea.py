...
def main():
    ...
    import sys
    process = subprocess.Popen([sys.executable, "src/app.py"], env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            shutil.copy(desktop_screenshot, os.path.join(proof_dir, 'desktop_no_phantom_textbox.png'))
            ...
            results["tests"].append({
                ...
                "assertions": [f"Mobile proxy input visibility on desktop: {is_visible} (expected False)"]
            })
            ...
            shutil.copy(mobile_typing, os.path.join(proof_dir, 'mobile_typing_buffer.png'))
            ...
            shutil.copy(mobile_after_backspace, os.path.join(proof_dir, 'mobile_after_backspace.png'))
        ...
        import traceback
        traceback.print_exc()
        results["tests"].append({"name": "textarea_tests", "status": "FAIL", "error": str(e)})
    finally:
