...
def test_debug_logging(page, server):
    ...
    assert enabled_log_count > initial_log_count, "No debug logs appeared after enabling"
    ...
    with open("docs/qa-images/test_debug_logs_output.txt", "w") as f:
        f.write(f"Test completed successfully. Debug logging toggle works. Enabled logs: {enabled_log_count - initial_log_count}, Disabled logs: {disabled_log_count}")
