...
def test_mobile_text_selection_overlay(mobile_page):
    ...
    start_new_btn = mobile_page.locator(
        ".connection-card .primary:has-text('Start New')"
    ).first
    start_new_btn.click()
...
def test_mobile_quick_tap_focus(mobile_page):
    ...
    mobile_page.wait_for_selector(
        ".mobile-selection-overlay", state="attached", timeout=10000
    )
    ...
    # Manually trigger a selection to clear later and verify it's not empty immediately
    selection_length_before = mobile_page.evaluate("""() => {
        const overlay = document.querySelector('.mobile-selection-overlay');
        if (overlay) {
    ...
    mobile_page.wait_for_function(
        "window.getSelection().toString().length === 0", timeout=5000
    )
...
def test_mobile_text_selection_alignment(mobile_page):
    ...
    if alignment_data["screenBox"]:
        assert (
            abs(
                alignment_data["screenBox"]["top"] - alignment_data["overlayBox"]["top"]
            )
            <= 1.0
        )
        assert (
            abs(
                alignment_data["screenBox"]["left"]
                - alignment_data["overlayBox"]["left"]
            )
            <= 1.0
        )
