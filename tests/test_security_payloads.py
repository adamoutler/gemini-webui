...
def test_add_ssh_key_text_invalid_format(client):
    payload = {
        "name": "invalid_key",
        "key": "some random text that is not a key"
    }
    response = client.post('/api/keys/text', json=payload)
...
def test_add_ssh_key_text_invalid_json(client):
    response = client.post('/api/keys/text', data="not json", content_type='application/json')
    assert response.status_code == 400

def test_upload_ssh_key_valid(client):
    data = {
        'file': (io.BytesIO(b"-----BEGIN OPENSSH PRIVATE KEY-----\nvalidkeydata\n"), 'valid_file.pem')
    }
    response = client.post('/api/keys/upload', data=data, content_type='multipart/form-data')
    assert response.status_code == 200

def test_upload_ssh_key_invalid_format(client):
    data = {
        'file': (io.BytesIO(b"some random text"), 'invalid_file.pem')
    }
    response = client.post('/api/keys/upload', data=data, content_type='multipart/form-data')
    assert response.status_code == 400
    assert b"Invalid SSH key format" in response.data
...
def test_upload_ssh_key_too_large(client):
    ...
    data = {
        'file': (io.BytesIO(large_content), 'large_file.pem')
    }
    response = client.post('/api/keys/upload', data=data, content_type='multipart/form-data')
    assert response.status_code == 400
    assert b"Payload too large" in response.data
...
def test_upload_ssh_key_missing_file(client):
    response = client.post('/api/keys/upload', data={}, content_type='multipart/form-data')
    assert response.status_code == 400
