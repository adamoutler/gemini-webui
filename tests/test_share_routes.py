...
def test_create_share_unauthenticated(client):
    with patch.object(type(env_config), 'BYPASS_AUTH_FOR_TESTING', new_callable=PropertyMock, return_value=False):
        response = client.post('/api/shares/create', json={
...
def test_share_lifecycle(client):
    ...
    with patch.object(type(env_config), 'BYPASS_AUTH_FOR_TESTING', new_callable=PropertyMock, return_value=True):
        # Create share
        create_resp = client.post('/api/shares/create', json={
