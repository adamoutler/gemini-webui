...
def run_test_with_viewport(server, width, height, expect_visible):
    with sync_playwright() as p:
        ...
        if expect_visible:
            ...
            assert box['height'] == 0, f"Textarea height should be 0 on desktop, but got: {box['height']}"
            assert box['width'] == 0, f"Textarea width should be 0 on desktop, but got: {box['width']}"
