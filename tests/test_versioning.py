...
def test_version_files_consistency(client):
    ...
    assert f'v{version}</div>' in html_content, f"Version {version} not found in UI response footer"
    
    # 3. Check manifest.json
    manifest_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'src', 'static', 'manifest.json')
    assert os.path.exists(manifest_path), "manifest.json does not exist"
    
    with open(manifest_path, 'r') as f:
        manifest_data = json.load(f)
        
    assert manifest_data.get('version') == version, f"manifest.json version {manifest_data.get('version')} does not match VERSION file {version}"
    
    # 4. Check sw.js
    sw_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'src', 'static', 'sw.js')
    ...
    assert expected_cache_name_pattern in sw_content, f"CACHE_NAME with version {version} not found in sw.js"
