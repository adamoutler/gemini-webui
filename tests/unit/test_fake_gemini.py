...
def test_fake_gemini_e2e_flow(page, server):
    ...
    print(rows)
    assert (
        "[Fake Gemini v2.0 - High Fidelity Mode]" in rows
        or "Welcome to Fake Gemini" in rows
        or "Hello Fake Gemini" in rows
    ), f"Expected Fake Gemini output not found. Got: {rows}"
