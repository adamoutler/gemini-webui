...
try:
    with open(
        "/home/adamoutler/.gemini/tmp/gemini-webui-1/tool-outputs/session-e2166e92-6bed-4128-8a43-865911fef931/mcp_kanban_list_work_items_1773261125563_0.txt",
        "r",
    ) as f:
        data = json.load(f)
    ...
    for item in data:
        ...
        if state_name in ["Backlog", "Todo", "In Progress"]:
            ...
            if "text" in name or "mobile" in name or "backspace" in name:
                print(
                    f"[{state_name}] GEMWEBUI-{item.get('sequence_id')}: {item.get('name')}"
                )
